...
(function () {
  ...
  /* ============================================================
     #access などのリンク先へ、描画のあとでもう一度合わせる
     ------------------------------------------------------------
     講話スケジュール・行事・活動報告は、このスクリプトが後から描いている。
     ブラウザは描く前の高さで #access へ飛ぶので、そのあとページが伸びると
     目的の見出しが下へずれ、**ひとつ上のQ&Aが見えてしまう**。
     公式LINEのリッチメニュー「会場アクセス」で実際に起きていた。
     描画のあとと、画像の読み終わりに、もう一度合わせ直す。
     ============================================================ */
  (function rehash() {
    ...
    function jump() {
      ...
      if (!el) return;
      // ここは合わせ直しなので一度で決める。
      // "auto" は CSS の scroll-behavior:smooth に従ってしまうため "instant" を使う
